...
class DeepLinkHandler: ObservableObject {
    ...
    // Nuevas propiedades para categorías y métodos de pago personalizados
    @Published var prefilledCustomCategory: String? = nil
    @Published var prefilledCustomPaymentMethod: String? = nil
    ...
    private func processAddGPDeepLink(_ url: URL) {
        ...
        if let components = URLComponents(url: url, resolvingAgainstBaseURL: true) {
            ...
            // Resetear valores personalizados antes de procesar
            prefilledCustomCategory = nil
            prefilledCustomPaymentMethod = nil
            
            // Extraer la categoría (estándar o personalizada)
            if let categoryItem = queryItems.first(where: { $0.name == "category" }),
               let categoryValue = categoryItem.value {
                if let standardCategory = GPCategory(rawValue: categoryValue) {
                    // Es una categoría estándar
                    prefilledCategory = standardCategory
                    prefilledCustomCategory = nil
                    print("DeepLinkHandler: Categoría estándar: \(standardCategory.rawValue)")
                } else {
                    // Es una categoría personalizada
                    prefilledCategory = .otros
                    prefilledCustomCategory = categoryValue
                    print("DeepLinkHandler: Categoría personalizada: \(categoryValue)")
                }
            } else {
                prefilledCategory = .otros
                prefilledCustomCategory = nil
            ...
            // Extraer el método de pago (estándar o personalizado)
            if let paymentMethodItem = queryItems.first(where: { $0.name == "paymentMethod" }),
               let methodValue = paymentMethodItem.value {
                if let standardMethod = PaymentMethod(rawValue: methodValue) {
                    // Es un método estándar
                    prefilledPaymentMethod = standardMethod
                    prefilledCustomPaymentMethod = nil
                    print("DeepLinkHandler: Método de pago estándar: \(standardMethod.rawValue)")
                } else {
                    // Es un método personalizado
                    prefilledPaymentMethod = .bank
                    prefilledCustomPaymentMethod = methodValue
                    print("DeepLinkHandler: Método de pago personalizado: \(methodValue)")
                }
            } else {
                prefilledPaymentMethod = .cash
                prefilledCustomPaymentMethod = nil
                print("DeepLinkHandler: Método de pago por defecto: Efectivo")
            }
