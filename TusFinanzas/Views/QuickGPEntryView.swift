...
struct QuickGPEntryView: View {
    ...
    let initialCustomCategory: String?
    let initialCustomPaymentMethod: String?
    ...
    init(isPresented: Binding<Bool>, initialAmount: Double, initialCategory: GPCategory, initialPaymentMethod: PaymentMethod = .cash, initialCustomCategory: String? = nil, initialCustomPaymentMethod: String? = nil, isQuickAmountEntry: Bool, viewModel: FinanceViewModel) {
        self._isPresented = isPresented
        self.initialAmount = initialAmount
        self.initialCategory = initialCategory
        self.initialPaymentMethod = initialPaymentMethod
        self.initialCustomCategory = initialCustomCategory
        self.initialCustomPaymentMethod = initialCustomPaymentMethod
        self._isQuickAmountEntry = State(initialValue: isQuickAmountEntry)
        self.viewModel = viewModel
        
        // Inicializar valores de categoría
        if let customCat = initialCustomCategory {
            self._category = State(initialValue: .otros)
            self._customGPCategory = State(initialValue: customCat)
            self._selectedCategoryString = State(initialValue: customCat)
        } else {
        ...
        // Inicializar valores de método de pago
        if let customMethod = initialCustomPaymentMethod {
            self._paymentMethod = State(initialValue: .bank)
            self._customPaymentMethod = State(initialValue: customMethod)
            self._selectedPaymentMethodString = State(initialValue: customMethod)
        } else {
            self._paymentMethod = State(initialValue: initialPaymentMethod)
            self._customPaymentMethod = State(initialValue: nil)
        ...
        var defaultConcept = ""
        if let customCat = initialCustomCategory {
            defaultConcept = customCat.capitalized
        } else {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    .onChange(of: selectedPaymentMethodString) { newValue in
                        ...
                        if let standardMethod = PaymentMethod.allCases.first(where: { $0.rawValue == newValue }) {
                            ...
                            // Es un método personalizado que usaremos directamente
    ...
    private func saveTransaction() {
        ...
        print("QuickGPEntryView: Categoría - \(customGPCategory ?? category.rawValue)")
        print("QuickGPEntryView: Método de pago - \(customPaymentMethod ?? paymentMethod.rawValue)")
    }
